...
def fitur_5():
    def input_progress():
        ...
        while True:
            ...
            if re.match(r"^(Day\s+[1-9]|Day\s+[1-9][0-9]|Day\s+[1-9][0-9][0-9]+|\d{4}-\d{2}-\d{2})$", session):
                if session.capitalize() in existing_sessions:
                    print(f"Session {session} sudah ada sebelumnya!!")
                    continue
                else:
                    break
            elif re.match(r"^(Day\s+-[1-9]|Day\s+-[1-9][0-9]|Day\s+-[1-9][0-9][0-9]+|\d{4}-\d{2}-\d{2})$", session):
